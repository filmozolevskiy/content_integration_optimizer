view: content_integration_optimizer {
 ...
 dimension: top_unbookable_revenue {
 type: number
 value_format: "#,##0.00"
 sql: (
 SELECT MAX(oc_unb.revenue)
 FROM ota.optimizer_candidates oc_unb
 WHERE oc_unb.attempt_id = ${TABLE}.attempt_id
 AND oc_unb.candidacy = 'Unbookable'
 AND oc_unb.created_at > ${start_date_bound}
 ) ;;
 group_label: "MONETARY"
 label: "Top Unbookable Revenue"
 description: "Highest revenue among candidates with candidacy='Unbookable' on this attempt, propagated to every row of the attempt. NULL when the attempt has no Unbookable candidates. Use as a dimension or filter (NOT a measure source) — summing across rows multi-counts per attempt. Same pattern as booked_contestant_revenue_on_attempt."
 }
 ...
 group_label: "4. TAGS"
 description: "Distinct Promoted tag values for this candidate, comma-separated. Source: ota.optimizer_candidate_tags joined to ota.optimizer_tags on name='Promoted' (same pattern as demoted_values). Observed values (7d): same, neutral, upgrade — one tag per candidate."
 }
